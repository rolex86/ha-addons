...
(function () {
  ...
  function setStatus(msg) {
    const el = $("status");
    if (el) el.innerHTML = 'Status: <strong>' + String(msg || "—") + '</strong>';
  }
  ...
  async function apiSaveConfig(payload) {
    ...
    if (!res.ok) throw new Error(data?.error || txt || ("Save failed: " + res.status));
    return data;
  }
  ...
  async function apiRestartAddon() {
    ...
    let data = null;
    try { data = txt ? JSON.parse(txt) : null; } catch { data = null; }
    if (!res.ok) throw new Error(data?.error || txt || ("Restart failed: " + res.status));
    return data;
  }

  async function apiGetTraktGenres(type) {
    const res = await fetch(`/api/trakt-genres?type=${encodeURIComponent(type)}`, {
      headers: headersWithToken()
    });
    if (!res.ok) {
      const txt = await res.text().catch(() => "");
  ...
  async function apiRunUpdate() {
    const res = await fetch("/api/run-update", { method: "POST", headers: headersWithToken() });
    const txt = await res.text().catch(() => "");
    let data = null;
    try { data = txt ? JSON.parse(txt) : null; } catch { data = null; }
    if (!res.ok) throw new Error(data?.error || txt || ("Run update failed: " + res.status));
    return data;
  }

  async function apiStopUpdate() {
    const res = await fetch("/api/stop-update", { method: "POST", headers: headersWithToken() });
    const txt = await res.text().catch(() => "");
    let data = null;
    try { data = txt ? JSON.parse(txt) : null; } catch { data = null; }
    if (!res.ok) throw new Error(data?.error || txt || ("Stop update failed: " + res.status));
    return data;
  }

  async function apiUpdateStatus() {
    const res = await fetch("/api/update-status", { headers: headersWithToken() });
    const txt = await res.text().catch(() => "");
    let data = null;
    try { data = txt ? JSON.parse(txt) : null; } catch { data = null; }
    if (!res.ok) throw new Error(data?.error || txt || ("Update status failed: " + res.status));
    return data;
  }
  ...
  function getGenresForType(type) {
    return (type === "series") ? (genresCache.series.items || GENRES_FALLBACK) : (genresCache.movie.items || GENRES_FALLBACK);
  }
  ...
  function renderListGenresUI() {
    ...
    for (const g of all) {
      ...
      span.textContent = name && name.toLowerCase() !== slug ? ("— " + name) : "";
      span.style.opacity = "0.75";

      cb.addEventListener("change", () => {
        const cur = new Set(parseCsv(hidden.value));
        if (cb.checked) cur.add(slug);
        else cur.delete(slug);
        setCsvToEl(hidden, Array.from(cur));
        renderListGenresUI();
      });

      label.appendChild(cb);
      label.appendChild(code);
      if (span.textContent) label.appendChild(span);
  ...
  function initListGenresUI() {
    ...
    on("btnAddGenre", "click", () => {
      const v = String($("genreCustom")?.value || "").trim().toLowerCase();
      if (!v) return;
      const cur = parseCsv($("f_genres").value);
      cur.push(v);
      setCsvToEl($("f_genres"), cur);
      if ($("genreCustom")) $("genreCustom").value = "";
      renderListGenresUI();
    });

    on("btnClearGenres", "click", () => {
      if ($("f_genres")) $("f_genres").value = "";
      renderListGenresUI();
    });
  ...
  function readDefaultsFromUI() {
    ...
    state.lists.defaults.candidatePages = Number($("d_candidatePages")?.value || 5);
    ...
    state.lists.defaults.dupRules.hardBlockTop = Number($("d_hardTop")?.value || 45);
    state.lists.defaults.dupRules.penaltyPerHit = Number($("d_penalty")?.value || 80);
  }
  ...
  function writeDefaultsToUI() {
    ...
    if ($("d_candidatePages")) $("d_candidatePages").value = d.candidatePages ?? 5;
  ...
  function writeSecretsToUI() {
    if ($("traktId")) $("traktId").value = state.secrets?.trakt?.client_id ?? "";
    if ($("traktSecret")) $("traktSecret").value = state.secrets?.trakt?.client_secret ?? "";
  }
  ...
  function renderListsTable() {
    ...
    for (const x of state.lists.lists) {
      ...
      tr.appendChild(td(x.filters?.genres || "", "mono"));
  ...
  function upsertList() {
    ...
    if (!id || !name) { alert("Vyplň id a name."); return; }
    ...
    const genres = String($("f_genres")?.value || "").trim();
    if (years) obj.filters.years = years;
    if (genres) obj.filters.genres = genres;
    ...
    for (const k in overrides) if (overrides[k] !== undefined) obj[k] = overrides[k];
  ...
  async function clearListForm() {
    ...
    const ids = ["candidatePages","pageLimit","finalSize","sleepMs","timeoutMs","hardTop","penalty","minRating","minCount","maxCount"];
    for (const k of ids) {
      const el = $("f_" + k);
      if (el) el.value = "";
    }
  ...
  async function loadListToForm(id) {
    ...
    fillSources();
    if ($("f_source")) {
      const fallback = (x.type === "series" ? SOURCES_SERIES[0].value : SOURCES_MOVIE[0].value);
      $("f_source").value = x.source?.path || fallback;
    }

    if ($("f_years")) $("f_years").value = x.filters?.years || "";
    if ($("f_genres")) $("f_genres").value = x.filters?.genres || "";

    if ($("f_candidatePages")) $("f_candidatePages").value = x.candidatePages ?? "";
  ...
  function ensureSmartPicksExists() {
    ...
    if (!Array.isArray(state.lists.smartPicks.profiles)) state.lists.smartPicks.profiles = [];
    if (typeof state.lists.smartPicks.enabled !== "boolean") state.lists.smartPicks.enabled = true;

    const ds = Number(state.lists.smartPicks.defaultSize);
    state.lists.smartPicks.defaultSize = Number.isFinite(ds) && ds >= 1 ? ds : 10;
  ...
  function normalizeSmartPickProfileOnRead(p) {
    ...
    out.type = (t === "series" || t === "show" || t === "shows") ? "series" : "movie";

    const sz = Number(out.size);
    out.size = (Number.isFinite(sz) && sz >= 1) ? sz : 10;

    // Legacy sources
    const fromTrakt = Array.isArray(out.fromTrakt) ? out.fromTrakt.map(safeStr).map(s => s.trim()).filter(Boolean) : [];
    const fromLists = Array.isArray(out.fromLists) ? out.fromLists.map(safeStr).map(s => s.trim()).filter(Boolean) : [];

    // "New-ish" sources (if present from past)
    const sTrakt = Array.isArray(out?.sources?.traktPaths) ? out.sources.traktPaths.map(safeStr).map(s => s.trim()).filter(Boolean) : [];
    const sLists = Array.isArray(out?.sources?.listIds) ? out.sources.listIds.map(safeStr).map(s => s.trim()).filter(Boolean) : [];
    ...
    // Legacy filters
    const filters = (out.filters && typeof out.filters === "object") ? out.filters : {};
  ...
  function toLegacySmartPickProfile(uiProfile, existingOnDisk) {
    const base = (existingOnDisk && typeof existingOnDisk === "object") ? { ...existingOnDisk } : {};
    ...
    const traktPaths = Array.isArray(uiProfile?.sources?.traktPaths) ? uiProfile.sources.traktPaths.map(safeStr).map(s => s.trim()).filter(Boolean) : [];
    const listIds = Array.isArray(uiProfile?.sources?.listIds) ? uiProfile.sources.listIds.map(safeStr).map(s => s.trim()).filter(Boolean) : [];
    ...
    const include = Array.isArray(uiProfile.includeGenres) ? uniqLower(uiProfile.includeGenres) : [];
    const genresCsv = include.length ? include.join(",") : "";
    if (genresCsv) out.filters.genres = genresCsv;
    else delete out.filters.genres;
  ...
  function writeSmartPicksTopToUI() {
    ...
    if ($("sp_defaultSize")) $("sp_defaultSize").value = String(state.lists.smartPicks.defaultSize ?? 10);
  }
  ...
  function setSpIncludeExcludeToHidden(include, exclude) {
    if ($("sp_includeGenres")) $("sp_includeGenres").value = uniqLower(include).join(",");
    if ($("sp_excludeGenres")) $("sp_excludeGenres").value = uniqLower(exclude).join(",");
  }
  ...
  function getSmartPickFromForm(validate = true) {
    ...
    const size = Number($("sp_size")?.value || state.lists.smartPicks?.defaultSize || 10);
    ...
    if (validate) {
      ...
      if (!["movie", "series"].includes(type)) throw new Error("SmartPicks profil: typ musí být movie/series.");
      if (!Number.isFinite(size) || size < 1) throw new Error("SmartPicks profil: počet musí být číslo >= 1.");
      if (years && !/^(\d{4})\s*-\s*(\d{4})$/.test(years)) throw new Error("SmartPicks profil: roky musí být ve formátu YYYY-YYYY nebo prázdné.");
    }

    const traktPaths = Array.from(document.querySelectorAll('#sp_src_trakt input[type="checkbox"]:checked'))
      .map(cb => String(cb.value || "").trim())
      .filter(Boolean);

    const listIds = Array.from(document.querySelectorAll('#sp_src_lists input[type="checkbox"]:checked'))
      .map(cb => String(cb.value || "").trim())
      .filter(Boolean);
  ...
  function renderSmartPicksSourcesUI() {
    ...
    const selectedTrakt = new Set((curProfile?.sources?.traktPaths || []).map(String));
    ...
    const selectedListIds = new Set((curProfile?.sources?.listIds || []).map(String));
  ...
  async function renderSmartPicksGenresUI() {
    ...
    const all = getGenresForType(type);
    const search = String($("sp_genreSearch")?.value || "").trim().toLowerCase();
    ...
    for (const g of all) {
      ...
      const st =
        setInc.has(slug) ? "include" :
        setExc.has(slug) ? "exclude" :
        "none";
      ...
      btn.addEventListener("click", () => {
        ...
        if (cur === "none") { setInc.add(slug); setExc.delete(slug); btn.dataset.state = "include"; }
        else if (cur === "include") { setInc.delete(slug); setExc.add(slug); btn.dataset.state = "exclude"; }
        else { setExc.delete(slug); setInc.delete(slug); btn.dataset.state = "none"; }
  ...
  function loadSmartPickToForm(profileId) {
    ...
    $("sp_size").value = String(p.size ?? (state.lists.smartPicks.defaultSize ?? 10));
  ...
  function upsertSmartPickProfile() {
    ...
    const idx = state.lists.smartPicks.profiles.findIndex(x => x.id === uiProfile.id);
  ...
  function deleteSmartPickProfile() {
    ...
    state.lists.smartPicks.profiles = state.lists.smartPicks.profiles.filter(p => p.id !== id);
    renderSmartPicksTable();
    clearSmartPickForm();
  ...
  async function loadFromDisk() {
    ...
    state.secrets = data.secrets || { trakt: { client_id: "", client_secret: "" } };

    state.lists.defaults = state.lists.defaults || {};
    state.lists.defaults.dupRules = state.lists.defaults.dupRules || {};
    state.lists.lists = Array.isArray(state.lists.lists) ? state.lists.lists : [];
  ...
  async function saveToDisk() {
    ...
    setStatus("saving…");
    const resp = await apiSaveConfig({ lists: state.lists, secrets: state.secrets });
      ...
      lists: [
        {
          ...
          filters: { years: "1985-2000", genres: "action,thriller,crime" },
  ...
  function setUpdateUIStatus(st) {
    ...
    if (updPill) updPill.innerHTML = `Update: <strong>${running ? "běží" : "stojí"}</strong>`;
  ...
  function openUpdateStream() {
    if (updateES) {
      try { updateES.close(); } catch {}
      updateES = null;
    }

    const token = String($("token")?.value || "").trim();
    const u = token ? `/api/update-stream?token=${encodeURIComponent(token)}` : "/api/update-stream";
    ...
    updateES.onmessage = (ev) => {
      try {
        ...
        if (msg?.progress) setUpdateUIStatus({ running: true, progress: msg.progress });
      } catch {
    ...
    updateES.addEventListener("done", (ev) => {
      ...
      setTimeout(async () => {
        try { setUpdateUIStatus(await apiUpdateStatus()); } catch {}
      }, 300);
    });
  ...
  on("btnSort", "click", () => {
    state.lists.lists.sort((a, b) => String(a.name || "").localeCompare(String(b.name || ""), "cs"));
    renderListsTable();
    renderSmartPicksSourcesUI();
  });
  ...
  on("btnSpAddSource", "click", () => {
    ...
    const exists = Array.from(seedBox.querySelectorAll('input[type="checkbox"]'))
      .some(cb => cb.value === v);
    ...
    if (!exists) {
      ...
      const cb = Array.from(seedBox.querySelectorAll('input[type="checkbox"]'))
        .find(cb => cb.value === v);
  ...
  (async () => {
    ...
    try {
      ...
      state.lists = { defaults: { dupRules: {} }, lists: [], smartPicks: { enabled: true, defaultSize: 10, profiles: [] } };
      state.secrets = { trakt: { client_id: "", client_secret: "" } };
